...
import { Plus, Send, Bot, Trash2, ChevronDown, ChevronUp, X, User, Loader2 } from 'lucide-react';
...
interface HubChatLangchainProps {
  agent: Agent;
  onClose?: () => void;
}

export const HubChatLangchain: React.FC<HubChatLangchainProps> = ({ agent, onClose }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log('[HubChatLangchain] Chat adapter initialized:', {
        framework: agent.framework,
        sessionId: currentSessionId,
        endpoint: agent.langchain_config?.endpoint,
        requestSchema: agent.langchain_config?.request_schema,
        responseFormat: agent.langchain_config?.response_format,
      });
    } catch (error) {
      console.error('[HubChatLangchain] Failed to create chat adapter:', error);
    }
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('[HubChatLangchain] Loaded sessions:', data.sessions?.length);
      }
    } catch (error) {
      console.error('[HubChatLangchain] Failed to load sessions:', error);
    } finally {
  ...
  const handleNewChat = () => {
    ...
    setMessages([]);
    console.log('[HubChatLangchain] Starting new chat');
  };
  ...
  const handleDeleteSession = async (sessionId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('[HubChatLangchain] Deleted session:', sessionId);
      }
    } catch (error) {
      console.error('[HubChatLangchain] Failed to delete session:', error);
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
        ...
        { content: userMessageContent },
        {
          ...
          onComplete: (response) => {
            ...
            if (response.reasoningContent) {
              ...
              setExpandedThinking((prev) => new Set(prev).add(assistantMessage.id));
            }
          ...
          onError: (error) => {
            console.error('[HubChatLangchain] Chat error:', error);
            setIsStreaming(false);
      ...
      console.error('[HubChatLangchain] Failed to send message:', error);
      setIsStreaming(false);
    }
  };
          ...
          {isLoadingMessages ? (
            ...
            <>
              {messages.map((message) => (
                <div key={message.id}>
                  {/* Reasoning/Thinking section (before assistant message) */}
                ...
                <div>
                  ...
                  {streamingReasoning && (
                    <div className="mb-4">
                      ...
                      >
                        <MessageContent content={streamingReasoning} contentType="markdown" />
                        <span className="inline-block h-3 w-1 animate-pulse ml-1" style={{ backgroundColor: '#9333ea' }} />
                      </div>
                    </div>
                  )}
